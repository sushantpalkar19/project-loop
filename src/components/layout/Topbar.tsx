...
import { Menu, LogOut, Shield, ChevronDown, Layers, Sparkles, Loader2 } from "lucide-react";
  ...
  return (
    <header className="sticky top-0 z-20 h-16 bg-white/95 backdrop-blur-md border-b border-slate-200/80 px-4 sm:px-8 flex items-center justify-between shadow-2xs">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-1.5">
              <span className="text-[11px] text-slate-400 font-semibold hidden sm:inline tracking-wide">
                Workspace
              </span>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          {/* Ask LOOP Quick Access - hidden on ask page itself */}
          {!isAskPage && (
            <Link href="/ask" className="hidden lg:block">
              <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-indigo-50 hover:bg-indigo-100 border border-indigo-200/80 text-indigo-700 text-xs font-semibold transition-all duration-150 shadow-2xs hover:shadow-sm group cursor-pointer">
                <Sparkles className="w-3.5 h-3.5 text-indigo-500 group-hover:text-indigo-700 transition-colors" />
                <span>Ask LOOP</span>
              </span>
            </Link>
          )}
          ...
          {/* Workspace Pill */}
          {user?.workspaceId && (
            <div className="hidden xl:flex items-center gap-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs shadow-2xs">
              <Layers className="w-3.5 h-3.5 text-indigo-600 shrink-0" />
              <span className="text-slate-400 font-semibold">Tenant:</span>
              <span className="font-mono text-slate-800 font-bold truncate max-w-[80px]">
          ...
          {user && (
            <div className="relative" ref={dropdownRef}>
              ...
              >
                <div className="w-8 h-8 rounded-full bg-indigo-600 text-white flex items-center justify-center font-bold text-xs shadow-sm ring-2 ring-indigo-100">
                ...
                <div className="absolute right-0 mt-2 w-60 rounded-2xl bg-white border border-slate-200 shadow-xl py-2 z-50 animate-in fade-in zoom-in-95 duration-100">
                  ...
                  <div className="pt-1">
                    <button
                      ...
                      className="w-full text-left px-4 py-2.5 text-xs text-rose-600 hover:bg-rose-50 flex items-center gap-2 font-semibold transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
